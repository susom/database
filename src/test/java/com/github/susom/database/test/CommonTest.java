...
public abstract class CommonTest {
  ...
  public void selectNewTable() {
    ...
    db.select("select nbr_integer, nbr_long, nbr_float, nbr_double, nbr_big_decimal, str_varchar, str_lob, "
      ...
      public Void process(Rows rs) throws Exception {
        assertTrue(rs.next());
        assertEquals(new Integer(1), rs.getInteger(1));
        assertEquals(new Integer(1), rs.getInteger("nbr_integer"));
        assertEquals(new Long(2), rs.getLong(2));
        assertEquals(new Long(2), rs.getLong("nbr_long"));
        assertEquals(new Float(3.2f), rs.getFloat(3));
        assertEquals(new Float(3.2f), rs.getFloat("nbr_float"));
        assertEquals(new Double(4.2), rs.getDouble(4));
        assertEquals(new Double(4.2), rs.getDouble("nbr_double"));
        assertEquals(new BigDecimal("5.3"), rs.getBigDecimal(5));
        assertEquals(new BigDecimal("5.3"), rs.getBigDecimal("nbr_big_decimal"));
        assertEquals("Hello", rs.getString(6));
        assertEquals("Hello", rs.getString("str_varchar"));
        assertEquals("World", rs.getClobString(7));
        assertEquals("World", rs.getClobString("str_lob"));
        assertArrayEquals("More".getBytes(), rs.getBlobBytes(8));
        assertArrayEquals("More".getBytes(), rs.getBlobBytes("bin_blob"));
        assertEquals(currentDate.getTime(), rs.getDate(9).getTime());
        assertEquals(currentDate.getTime(), rs.getDate("date_millis").getTime());
        return null;
      }
    });
    db.select("select str_lob, bin_blob from dbtest").query(new RowsHandler<Void>() {
      ...
      public Void process(Rows rs) throws Exception {
        assertTrue(rs.next());
        assertEquals("World", readerToString(rs.getClobReader(1)));
        assertArrayEquals("More".getBytes(), inputStreamToString(rs.getBlobInputStream(2)));
        return null;
      }
    });
    db.select("select str_lob, bin_blob from dbtest").query(new RowsHandler<Void>() {
      ...
      public Void process(Rows rs) throws Exception {
        assertTrue(rs.next());
        assertEquals("World", readerToString(rs.getClobReader("str_lob")));
        assertArrayEquals("More".getBytes(), inputStreamToString(rs.getBlobInputStream("bin_blob")));
        return null;
      }
    });
  ...
  public void updatePositionalArgs() {
    ...
    db.select("select nbr_integer, nbr_long, nbr_float, nbr_double, nbr_big_decimal, str_varchar, str_lob, "
      ...
      public Void process(Rows rs) throws Exception {
        assertTrue(rs.next());
        assertNull(rs.getInteger(1));
        assertNull(rs.getInteger("nbr_integer"));
        assertNull(rs.getLong(2));
        assertNull(rs.getLong("nbr_long"));
        assertNull(rs.getFloat(3));
        assertNull(rs.getFloat("nbr_float"));
        assertNull(rs.getDouble(4));
        assertNull(rs.getDouble("nbr_double"));
        assertNull(rs.getBigDecimal(5));
        assertNull(rs.getBigDecimal("nbr_big_decimal"));
        assertNull(rs.getString(6));
        assertNull(rs.getString("str_varchar"));
        assertNull(rs.getClobString(7));
        assertNull(rs.getClobString("str_lob"));
        assertNull(rs.getBlobBytes(8));
        assertNull(rs.getBlobBytes("bin_blob"));
        assertNull(rs.getDate(9));
        assertNull(rs.getDate("date_millis"));
        return null;
      }
    });
    ...
    db.select("select str_lob, bin_blob from dbtest").query(new RowsHandler<Void>() {
      ...
      public Void process(Rows rs) throws Exception {
        assertTrue(rs.next());
        assertNull(rs.getClobString(1));
        assertNull(rs.getClobString("str_lob"));
        assertNull(rs.getBlobBytes(2));
        assertNull(rs.getBlobBytes("bin_blob"));
        return null;
      }
    });
    ...
    db.select("select str_lob, bin_blob from dbtest").query(new RowsHandler<Void>() {
      ...
      public Void process(Rows rs) throws Exception {
        assertTrue(rs.next());
        assertEquals("World", rs.getClobString(1));
        assertEquals("World", rs.getClobString("str_lob"));
        assertArrayEquals("More".getBytes(), rs.getBlobBytes(2));
        assertArrayEquals("More".getBytes(), rs.getBlobBytes("bin_blob"));
        return null;
      }
    });
  ...
  public void nullValues() {
    ...
    db.select("select str_lob, bin_blob from dbtest").query(new RowsHandler<Void>() {
      ...
      public Void process(Rows rs) throws Exception {
        assertTrue(rs.next());
        assertNull(rs.getClobReader(1));
        assertNull(rs.getBlobInputStream(2));
        return null;
      }
    });
    db.select("select str_lob, bin_blob from dbtest").query(new RowsHandler<Void>() {
      ...
      public Void process(Rows rs) throws Exception {
        assertTrue(rs.next());
        assertNull(rs.getClobReader("str_lob"));
        assertNull(rs.getBlobInputStream("bin_blob"));
        return null;
      }
    });
  }
  ...
  public void bigClob() {
    ...
    db.select("select str_lob from dbtest").query(new RowsHandler<Void>() {
      ...
      public Void process(Rows rs) throws Exception {
        assertTrue(rs.next());
        assertEquals(longString, rs.getClobString(1));
        assertEquals(longString, rs.getClobString("str_lob"));
        assertEquals(longString, readerToString(rs.getClobReader(1)));
        return null;
      }
    });
    ...
    db.get().select("select str_lob from dbtest").query(new RowsHandler<Void>() {
      ...
      public Void process(Rows rs) throws Exception {
        assertTrue(rs.next());
        assertEquals(longString, readerToString(rs.getClobReader("str_lob")));
        return null;
      }
    });
  ...
  public void bigBlob() {
    ...
    db.select("select bin_blob from dbtest").query(new RowsHandler<Void>() {
      ...
      public Void process(Rows rs) throws Exception {
        assertTrue(rs.next());
        assertArrayEquals(bigBytes, rs.getBlobBytes(1));
        assertArrayEquals(bigBytes, rs.getBlobBytes("bin_blob"));
        assertArrayEquals(bigBytes, inputStreamToString(rs.getBlobInputStream(1)));
        return null;
      }
    });
    db.select("select bin_blob from dbtest").query(new RowsHandler<Void>() {
      ...
      public Void process(Rows rs) throws Exception {
        assertTrue(rs.next());
        assertArrayEquals(bigBytes, inputStreamToString(rs.getBlobInputStream("bin_blob")));
        return null;
      }
    });
    db.delete("delete from dbtest").update(1);
