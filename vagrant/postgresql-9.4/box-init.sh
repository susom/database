...
sudo apt-get update && apt-get install -y postgresql-9.4
sudo -u postgres echo "listen_addresses='*'" >> /etc/postgresql/9.4/main/postgresql.conf
sudo -u postgres echo "host all all 0.0.0.0/0 md5" >> /etc/postgresql/9.4/main/pg_hba.conf
